...
export function getToday(
  type: 'timestamp' | 'timestamp_string' | 'month' | '' = '',
...
export function getDatesBetween(
  ...
  const endDate = typeof stopDate === 'string' ? new Date(stopDate) : stopDate;
 ...
 * @returns {Date}
 */
export function subtractTimePeriod(
  date: string | Date,
...
export function setDates(quick_option: string): startEndDates {
  const today = new Date();
  const this_week_number = getWeekNumber(today);
  const this_year = getThisYear();
  const last_year = this_year - 1;
  const this_month = today.getMonth(); // 0-11
  const last_month = this_month === 0 ? 12 : this_month - 1;
  const next_month = this_month === 11 ? 1 : this_month + 1;
  let start_date;
  let end_date;
  switch (quick_option) {
    case 'all':
    ...
    case 'last_week':
      ...
      start_date = formatDate(
        addDaystoDate(
          -6,
          formatDate(getFirstDayOfWeek(this_week_number, this_year))
        )
      );
      end_date = formatDate(addDaystoDate(6, start_date));
      break;
    ...
    case 'this_month':
      ...
      end_date = formatDate(
        new Date(this_year, this_month, getLastDayOfMonth(this_month))
      );
      break;
    case 'last_month':
      start_date = formatDate(
        new Date(new Date(this_year, this_month, 1).setMonth(this_month - 1))
      );
      end_date = formatDate(
        new Date(
          new Date(
            this_year,
            this_month,
            getLastDayOfMonth(last_month)
          ).setMonth(this_month - 1)
        )
      );
      break;
    case 'next_month':
